...
@pytest.mark.asyncio
async def test__get_joke__returns_200__when_use_case_returns_a_joke(mocker):
    use_case_mock = mocker.Mock(return_value="joke")
    response: JSONResponse = await joke_router.get_joke(use_case_mock)
...
async def test__get_joke__returns_joke_data_when_use_case_returns_a_joke(mocker, faker):
    ...
    use_case_mock = mocker.Mock(return_value=random_joke)
...
async def test__get_joke__returns_500__when_use_case_raises_an_exception(mocker):
    ...
    use_case_mock = mocker.Mock(
        return_value=expected_response, side_effect=core.GetJokeException("Error getting joke")
    )
